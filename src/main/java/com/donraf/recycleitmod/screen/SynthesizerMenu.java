...
public class SynthesizerMenu extends AbstractContainerMenu {
    ...
    public SynthesizerMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.SYNTHESIZER_MENU.get(), pContainerId);
        checkContainerSize(inv, 1);
        blockEntity = ((SynthesizerBlockEntity) entity);
    ...
    public void addRecyclePoints(int recyclePoints) {
        data.set(2, data.get(2) + recyclePoints);
    }
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.SYNTHESIZER_BLOCK.get());
    }
